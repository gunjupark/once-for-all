...
parser.add_argument('--task', type=str, default='super', choices=[
    'super', 'kernel', 'depth', 'expand',
])
...
if args.task == 'super':
    ...
    args.path = 'exp/k2kd/phase%d' % args.phase
    args.dynamic_batch_size = 4
    if args.phase == 1:
        args.n_epochs = 25
        args.base_lr = 5e-4
    ...
    args.path = 'exp/kd2kdw/phase%d' % args.phase
...
if __name__ == '__main__':
    ...
    if args.task == 'super':
        ...
        if distributed_run_manager.start_epoch == 0:
            #model_path = download_url('https://hanlab.mit.edu/files/OnceForAll/ofa_checkpoints/ofa_D4_E6_K7', model_dir='.torch/ofa_checkpoints/%d' % hvd.rank())
            model_path = 'exp/super/%d/model_best.pth.tar' % hvd.rank()
            load_models(distributed_run_manager, distributed_run_manager.net, model_path=model_path)
            distributed_run_manager.write_log('%.3f\t%s\t%s\t%s' %
                                              validate_bp(distributed_run_manager, **validate_func_dict), 'valid')
        train(distributed_run_manager, args,
              lambda _run_manager, epoch, is_test: validate_bp(_run_manager, epoch, is_test, **validate_func_dict))
